# God-Museum

[![License](https://img.shields.io/badge/license-Concept%20Only-lightgrey)](./LICENSE)
[![Made with Markdown](https://img.shields.io/badge/Made%20with-Markdown-blue)](https://www.markdownguide.org)
[![GitHub stars](https://img.shields.io/github/stars/realchendahuang/god-museum?style=social)](https://github.com/realchendahuang/god-museum/stargazers)
[![Last commit](https://img.shields.io/github/last-commit/realchendahuang/god-museum)](https://github.com/realchendahuang/god-museum/commits)
[![Issues](https://img.shields.io/github/issues/realchendahuang/god-museum)](https://github.com/realchendahuang/god-museum/issues)

**English** · [中文](./README.md)

---

## What this is

Take the *Classic of Mountains and Seas*, *Journey to the West*, the Greek, Norse, and Egyptian pantheons — systems that have nothing to do with each other — and fit them into one shared cosmos, then make them not fight. The same lightning principle manifests as Zeus in Greece and as Leigong in the East. The same world-drowning flood is remembered by the Chinese as Nüwa patching the sky and by the Hebrews as Noah's ark. Take those questions seriously and the whole mythological landscape gets rearranged.

This repo holds the rearranged version. It's a setting bible, a project charter, and the foundation of a digital culture IP. Game writers can lift it as a world, novelists can lift it as a backbone, a digital exhibition can lift it for the visuals. Star it before you lift it.

## Multiple realms in parallel, joined by Gates

The Nine Heavens, Olympus, the Yellow Springs, Hades, Niflheim — in this museum they're all real, parallel layers, normally blind to one another, connected at a handful of points called Gates. That lets Set wander into the East as a foreign plague-god, lets Mjölnir crack open a rift into the underworld, lets the sun Kuafu chases each night plunge into a trench in Niflheim. "He came from the West." "She descended to the underworld." Those lines in mythology describe crossings between layers. Taking them as cross-layer movement, literally, holds up much better than flattening the gods onto one map.

The protocol below keeps this design standing.

## The Three Axioms

- **The Primordial Source.** Every mythology begins with the same explosion. Pangu splitting heaven from earth and Chaos giving form to the Greek cosmos are the same instant of creation, witnessed and told separately by the East and by Hellas. A god, in this project, is a cosmic principle that has taken on a body inside one specific layer.
- **Resonance of Domains.** Lightning, the sea, death, the harvest — each domain is one thing, manifested in different layers as Zeus, Poseidon, or Gonggong and Leigong. Within its own layer each is a true god. When two of them meet, they resonate. Eastern and Western thunder gods behave suspiciously alike because they share one principle.
- **Epic Convergence.** The flood Nüwa patched the sky against, the flood Noah rode out, the flood Gilgamesh survived — the same flood. Similarities this large are treated as evidence, not coincidence. They're era-level events that spread through the Gates into every layer at once.

On top of the three axioms sits one thermodynamic law: **belief is the energy source of divine power.** When the incense stops and the worshippers drift away, the godhead begins to decay — eventually dropping into the archive cabinet of the Fallen Mythos, the museum's back room where the old gods and the Elder Outer Ones sit in the dark.

## The Five Dossiers

Content is grouped by dossier, not by country. The point of a museum is to see how artifacts from different civilizations look side by side in the same room.

**01 · Worldview** — the axioms worked out, the ten-layer map with Gate coordinates, the thermodynamics of belief. This dossier sets the tone; the other four grow out of it.

**02 · Pantheon** — gods grouped by domain and alignment, not by nation.
- Creators and higher-dimensional beings: Pangu / Chaos / Ymir
- Powers of Nature: Sun gods (Apollo · Xihe · Ra), Thunder gods (Thor · Zeus · Leizhenzi)
- Lords of Death: Yanluo Wang / Hades / Anubis
- Each god gets one entry: titles, office, origin, which layer they live in, and what happens when they cross over. Domain resonances and godhead fusions get logged here too.

**03 · Containment** — non-human creatures treated as anomalies produced by laws spilling over or slipping out of alignment. Extensions of a god's will leaking past its body, or a natural disaster given shape.
- Eastern spine: the *Classic of Mountains and Seas* — Qiongqi, Taotie, the nine-tailed fox
- Western spine: Greek and Norse — Medusa, Argus, Fenrir
- The same dossier also contains the Fallen Mythos — gods whose belief hit zero and Outer Ones that fell out of the pantheon.

**04 · Armory** — the artifacts that actually rewrote the course of the pantheons, with attributes and physical explanations.
- Eastern treasures: the Ocean-Calming Pillar / Xuanyuan Sword / the Demon-Refining Pot / the Investiture Scroll
- Western relics: Mjölnir / Gungnir / the Fates' threads / Pandora's box
- Once enough of them are catalogued, open the "Cross-Layer Weapon Collider" — a sandbox for questions like what actually happens when the Ruyi Jingu Bang hits the Aegis. The collider answers that question as part of the setting.

**05 · Chronosphere** — every mythological plot gets jammed onto a single timeline that converges toward an absolute cosmic timeline. This is the part that makes the universe stand up. Three eras in, the pantheon has already been rearranged:

```
Chaos & Origin → Order & War  → Great Cataclysm → Margin of Gods & Men
 Titans           Titanomachy      The Flood Era    Rise of demigods
                  Battle of Zhuolu                  Trojan War · Fengshen
```

## Directory Plan

```
god-museum/
├── 📁 01_Worldview (协议基石)
│   ├── The_Axioms.md              # 宇宙三大法则推演
│   ├── Multiverse_Topology.md     # 十维界层地图与界门坐标
│   └── Belief_Thermodynamics.md   # 信仰热力学与神力维系机制
├── 📁 02_Pantheon (万神殿)
│   ├── Primes.md                  # 创世神与高维存在
│   └── Resonance_Log.md           # 权柄共振与神格融合记录
├── 📁 03_Containment (异常收容)
│   ├── Glitch_Bestiary.md         # 异兽与法则错误生物
│   └── The_Fallen_Mythos.md       # 死寂神系与旧日支配者
├── 📁 04_Armory (因果兵器)
│   ├── Relics.md                  # 圣物属性与物理学解释
│   └── The_Collider.md            # 💥 跨界兵器对撞沙盘实验
├── 📁 05_Chronosphere (统一编年史)
│   └── Absolute_Timeline.md       # 宇宙终极时间线收束
├── README.md
└── README_EN.md
```

This will shift as the content gets broken down. The skeleton above is where it starts.

## License

The setting itself is not openly licensed. Talk to us before lifting it.

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=realchendahuang/god-museum&type=Date)](https://star-history.com/#realchendahuang/god-museum&Date)